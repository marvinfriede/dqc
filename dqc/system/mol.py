...
def _parse_basis(
    atomzs: torch.Tensor, basis: BasisInpType, normalized: bool = False
) -> List[List[CGTOBasis]]:
    ...
    if isinstance(basis, str):
        return [
            loadbasis("%d:%s" % (atomz, basis), normalized=normalized)
            for atomz in atomzs
        ]
...
def _get_nelecs_spin(
    ...
    if spin is None:
        ...
        if not frac_mode:
            ...
            assert (
                nelecs - spin
            ) % 2 == 0, "Spin %d is not suited for %d electrons" % (spin, nelecs)
